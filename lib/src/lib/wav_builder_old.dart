...
class WavBuilderOld {
  ...
  WavBuilderOld(List<BlockBase> blocks, this._frequency, this._amplifySoundSignal,
      this._progress) {
    _blocks = blocks;
    if (_frequency < 11025)
  ...
  void _addBlockSoundData(BlockBase block) {
    ...
    if (block is DataBlock) {
      ...
      for (var i = 0; i < block.pulses; i++) {
        _doSignal(signalState, block.pulseLen);
        signalState = signalState == hi ? lo : hi;
      }
      if (signalState == lo) _doSignal(lo, block.pulseLen);
